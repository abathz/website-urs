...
{
	define('SANGAR_SLIDER_VERSION','1.2.5');
...
/**
 * Sangar slider promo page
 */
add_action('plugins_loaded','sangar_slider_lite_promo_page');
function sangar_slider_lite_promo_page() 
{
	// License page submenu
	add_action('admin_menu', 'sslider_promo_menu' );
	function sslider_promo_menu() {	
		add_submenu_page('sangar_slider_admin', 		
			'Upgrade to Pro', 
			'Upgrade to Pro',
			'moderate_comments',
			'sslider_promo_page',
			'sslider_promo_page_callback');	
	}
	function sslider_promo_page_callback() {	
		require_once( plugin_dir_path( __FILE__ ) . 'promo-page.php');
	}
}
...
function sslider_lite_notice() 
{
    ...
    if (! $is_ignore_notice  && ! function_exists("is_sslider_lite_exist")) 
    {
        ...
        printf(__('Unlock more preset , themes and layer editor. %1$s Get all features of Sangar Slider Pro ! %2$s Do not bug me again %3$s Not Now %4$s',SANGAR_SLIDER_VERSION), 
...
function sslider_lite_nag_ignore() 
{
    ...
    else if (isset($_GET['sslider_lite_nag_ignore']) && $_GET['sslider_lite_nag_ignore'] == 'later') 
    {
