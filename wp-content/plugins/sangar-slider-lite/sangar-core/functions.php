...
/**
 * Register dummy post type
 */
add_action('init', 'sslider_register_dummy_post_type');
function sslider_register_dummy_post_type()
{
    $post_type = 'sslider-dummy';
    $taxonomy = 'sslider-dummy-taxonomy';
    $sslider_addons = apply_filters('sangar_slider_addons',array());
    $needed_dummy = array('sangar_slider_post','sangar_slider_shop');

    // if sangar slider post addon is available
    if(! array_key_exists('sangar_slider_post', $sslider_addons)) return;

    // register_post_type
    register_post_type($post_type, array(
        'labels' => array(
            'name' => 'Dummy Posts for Sangar Slider',
            'singular_name' => 'Dummy Posts',
            'menu_name' => 'Dummy Posts',
            'all_items' => 'All Items',
        ),
        'description' => 'Dummy posts used for sangar slider post preset example.',
        'public' => true,            
...
function sslider_custom_menu_order($menu_ord)
{
    ...
    if(! isset($submenu['sangar_slider_admin'])) return;
    ...
    foreach($submenu['sangar_slider_admin'] as $key => $value)
    {
        ...
        else if($value[0] == 'Add Basic Slider' && $key != 1)
        {
...
function sslider_submenu_make_active() 
{
    ...
    if(! is_string($post_type)) return;

    $screen = get_current_screen();

    // return on add
    if(isset($screen->action) && $screen->action == 'add') return;

    // get all addons
    $sslider_addons = apply_filters('sangar_slider_addons',array());

    if(! array_key_exists($post_type, $sslider_addons)) return;
 ...
 */
add_filter('sangar_slider_delete_query', function($data){
    $key = $data[0];
    $url = $data[1];
