...
if ( ! class_exists('TopMusicChartsWidget')) {
	class TopMusicChartsWidget extends WP_Widget
	{
		...
		{
			// outputs the content of the widget
			$feed = $this->fetch_feed('http://www.billboard.com/rss/charts/' . $instance['chart']);
					...
					?>
						<ol>
							<?php foreach($rss_items as $item): ?>
								<li>
									<div><strong><?php echo $item->get_item_tags('', 'chart_item_title')[0]['data']; ?></strong></div>
									<div><small><?php echo $item->get_item_tags('', 'artist')[0]['data']; ?></small></div>
								</li>
							<?php endforeach ?>
						</ol>
					<?php
		...
		public function form($instance)
		{
			?>
						...
						<?php foreach ($this->charts as $group_name => $group): ?>
							<optgroup label="<?php echo $group_name; ?>">
								<?php foreach ($group as $chart_name => $url): ?>
									<option value="<?php echo $url; ?>" <?php echo $instance['chart'] === $url ? 'selected="selected"' : ''; ?>><?php echo $chart_name; ?></option>
								<?php endforeach ?>
							</optgroup>
						<?php endforeach ?>
					</select>
				...
				<p>
					...
					<input type="number" step="1" min="1" size="3" class="tiny-text" id="<?php echo $this->get_field_id('max'); ?>" name="<?php echo $this->get_field_name('max'); ?>" value="<?php echo esc_attr($instance['max']); ?>" />
				</p>
			<?php
		}
