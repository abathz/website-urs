...
Class ssliderGenerateAddonPost extends ssliderGenerate
{
	...
	function html()
	{
		...
		foreach ($this->data as $key => $value) 
		{
			...
			while ($the_query->have_posts())
			{
				...
				$page_counter++;
				$layer_data = $value['slide-layer'];
				$layer_data = $this->apply_shortcode($layer_data,$page_counter);

				$layer_data = base64_encode($layer_data);
		...
		// add loading
		$loading_attr = 'style="display:block;position:absolute;width:100%;height:100%;z-index:99;top:0px;left:0px;background:rgb(34,34,34);"';
	...
	private function apply_shortcode($layer_data,$page_counter)
	{
		...
		$layer_data = preg_replace_callback("/\[sslider-featured-image\s*=\s*\\\?[\"|'](.+?)\\\?[\"|']\s*\]/", 
			function ($matches)
			{
		        ...
		        $src = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()),$size)[0];
		        $image = "<img src='$src'>";
	...
	function javascript()
	{
		...
		$javascript.= "onResetInternal : function(base) { \r\n";
